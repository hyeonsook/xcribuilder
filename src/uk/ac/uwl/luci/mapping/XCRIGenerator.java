...
public class XCRIGenerator extends TimerTask
{
    ...
    private boolean callDummyTimer = false;
    ...
    public void start()
    {
        callDummyTimer = true;
        timer = new Timer();
        Logger.getLogger(XCRIGenerator.class.getName()).log(Level.INFO, (new StringBuilder()).append("Starting timer. The configured start date is ").append(month + 1).append("/").append(day).append(" (mm/dd).").toString());
    ...
    public void run()
    {
        try {
            ...
            {
                String targetFolder = xcriObj.getProp().getProperty("system.target.location");
                if (targetFolder == null){
                    targetFolder = System.getProperty("user.dir")+"/";
                    System.out.println("property file path.. :"+targetFolder);
                }
                cat.save(new File((new StringBuilder()).append(targetFolder).append("xcri").append((new SimpleDateFormat("yyyy-MM-dd")).format(Calendar.getInstance().getTime())).append(".xml").toString()), opts);
                cat.save(new File((new StringBuilder()).append(targetFolder).append("xcri.xml").toString()), opts);
                ...
                Logger.getLogger(XCRIGenerator.class.getName()).log(Level.INFO, "RDF generator start option: "+prop.getProperty("rdf.generate") );                
            ...
            //Call Dummy Timer to keep hibernate session alive
            if(callDummyTimer){
                new SessionKeeper(xcriObj.getController()).start();
                callDummyTimer = false;
            }
    ...
    {
        Logger.getLogger(XCRIGenerator.class.getName()).log(Level.INFO, "XcriGenerator started." +args.length);
        if(args.length <= 0)
        {
            printHelp();
        } else
        {
            ...
            if(command.equals("start") && args.length == 2 && isDate(args[1]))
            {
                ...
                //Logger.getLogger(XCRIGenerator.class.getName()).log(Level.INFO, null, "The xcribuilder is going to start on "+ args[1]);
            } else if(command.equals("run"))
            {
                
                //XCRIGenerator xcriGenerator = getInstance(""+Calendar.getInstance().get(Calendar.MONTH)+1, ""+Calendar.getInstance().get(Calendar.DAY_OF_MONTH));
                //xcriGenerator.start();
    ...
    public static void printHelp()
    {
        ...
        System.out.println("   e.g. java -jar XcriBuilder.jar start 01/01/12");
